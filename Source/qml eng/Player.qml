...
Image{
        ...
        Setting{
            ...
            function boalSet(){
                ...
                default: return "/image/res/Player.png";
                }
            }
            function weitSeit(){
                switch(boal){
    ...
    Timer{
        ...
        interval: (settings.defSet()-lvl*200)/20;
        repeat:true;
        onTriggered: {
    ...
    Timer{
        ...
        interval: 16;
        repeat: true;
        function movNormalize(){
            if(mov>1) mov=1;
            if(mov<-1) mov=-1;
            if(movL==false&&movR==false&&mode==0){
                if(mov>0){
        ...
        onTriggered: {
            record+=(lvl+lvl*effects_gold)*(settings.def+1);
             if(movD)
                 downm+=settings.weitSeit()/70;
             else
                 downm=0;
